...
export default function About() {
  return (
    <Layout pageName="About">
      <div className="p-6 gap-8 bg-gray-100 min-h-screen w-full flex justify-center">
        <div className="bg-white border rounded-sm p-5 w-full max-w-3xl shadow-lg">
          <h1 className="text-3xl font-[Racing_Sans_One] text-gray-700 mb-2">
            How To Play
          </h1>
          ...
          <table className="table-auto mb-4">
            <thead className="border-b">
              <tr>
                <th className="text-gray-700 px-3 py-2 text-left">Position</th>
                <th className="text-gray-700 px-3 py-2 text-left">Points</th>
              </tr>
            </thead>
            <tbody>
              {Object.entries(POSITION_POINTS).map(([pos, pts]) => (
                <tr
                  key={pos}
                  className="border-b odd:bg-gray-50 hover:bg-red-50 transition duration-150"
                >
                  <td className="px-3 py-1 text-gray-600 text-sm">{ORDINALS[+pos]}</td>
                  <td className="px-3 py-1 text-gray-600 text-sm font-semibold">{pts}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <p className="text-gray-600">Deal with it—or call me</p>
        </div>
      </div>
    </Layout>
  );
