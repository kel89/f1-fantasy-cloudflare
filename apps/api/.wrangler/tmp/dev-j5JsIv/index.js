...
// .wrangler/tmp/bundle-ymRyQ0/strip-cf-connecting-ip-header.js
...
// ../../node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260301.1/node_modules/wrangler/_virtual_unenv_global_polyfill-@cloudflare-unenv-preset-node-console
...
// ../../node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260301.1/node_modules/wrangler/_virtual_unenv_global_polyfill-@cloudflare-unenv-preset-node-process
...
// ../../node_modules/.pnpm/hono@4.12.3/node_modules/hono/dist/middleware/secure-headers/secure-headers.js
var HEADERS_MAP = {
...
// ../../node_modules/.pnpm/@hono+zod-validator@0.4.3_hono@4.12.3_zod@3.25.76/node_modules/@hono/zod-validator/dist/index.js
...
async function hashPassword(password) {
  ...
  const bits = await crypto.subtle.deriveBits(
    { name: "PBKDF2", salt: salt.buffer, iterations: ITERATIONS, hash: HASH },
    keyMaterial,
    KEY_LENGTH * 8
  );
...
function getAccessKey(secret) {
  return encoder2.encode(secret);
}
__name(getAccessKey, "getAccessKey");
function getRefreshKey(secret) {
  return encoder2.encode(secret);
}
__name(getRefreshKey, "getRefreshKey");
async function signAccessToken(payload, secret) {
  return new SignJWT({ ...payload, type: "access" }).setProtectedHeader({ alg: "HS256" }).setIssuedAt().setExpirationTime("15m").sign(getAccessKey(secret));
}
__name(signAccessToken, "signAccessToken");
async function signRefreshToken(payload, secret) {
  return new SignJWT({ ...payload, type: "refresh" }).setProtectedHeader({ alg: "HS256" }).setIssuedAt().setExpirationTime("30d").sign(getRefreshKey(secret));
}
...
var requireAdmin = /* @__PURE__ */ __name(async (c, next) => {
...
function rateLimit(max, windowSeconds) {
  return async (c, next) => {
    ...
    await c.env.JWT_BLACKLIST.put(key, String(count3 + 1), {
...
var signupSchema = external_exports.object({
  ...
  given_name: external_exports.string().min(1).max(100).trim(),
  family_name: external_exports.string().min(1).max(100).trim(),
  nickname: external_exports.string().min(1).max(50).trim()
});
...
auth.post("/signup", zValidator("json", signupSchema), async (c) => {
  const { email, password, given_name, family_name, nickname } = c.req.valid("json");
  ...
  await c.env.DB.prepare(
    `INSERT INTO users (id, email, given_name, family_name, nickname, password_hash)
     VALUES (?, ?, ?, ?, ?, ?)`
  ).bind(id, email.toLowerCase(), given_name, family_name, nickname, password_hash).run();
...
auth.patch("/me", requireAuth, zValidator("json", updateMeSchema), async (c) => {
  ...
  await c.env.DB.prepare("UPDATE users SET nickname = ? WHERE id = ?").bind(nickname, userId).run();
...
races.get("/:id", async (c) => {
  ...
  const race = await c.env.DB.prepare("SELECT * FROM races WHERE id = ?").bind(raceId).first();
  if (!race) {
  ...
  const rosters2 = await c.env.DB.prepare(
  ...
  const formattedRosters = rosters2.results.map((r) => ({
    id: r.id,
    user_id: r.user_id,
    race_id: r.race_id,
    driver_order: JSON.parse(r.driver_order || "[]"),
...
var patchRaceSchema = external_exports.object({
  name: external_exports.string().min(1).max(200).optional(),
  city: external_exports.string().min(1).max(100).optional(),
  country: external_exports.string().min(1).max(100).optional(),
  race_date: external_exports.string().datetime().optional(),
  status: external_exports.enum(["upcoming", "locked", "scored"]).optional()
});
races.patch("/:id", requireAdmin, zValidator("json", patchRaceSchema), async (c) => {
  const raceId = c.req.param("id");
  const body = c.req.valid("json");
  const existing = await c.env.DB.prepare("SELECT id FROM races WHERE id = ?").bind(raceId).first();
  if (!existing) {
    return c.json({ error: "Race not found", code: "NOT_FOUND" }, 404);
  }
  const fields = Object.entries(body).filter(([, v]) => v !== void 0);
  if (fields.length === 0) {
    return c.json({ error: "No fields to update", code: "VALIDATION_ERROR" }, 400);
  }
  const setClauses = fields.map(([k]) => `${k} = ?`).join(", ");
  const values = fields.map(([, v]) => v);
  await c.env.DB.prepare(`UPDATE races SET ${setClauses} WHERE id = ?`).bind(...values, raceId).run();
  const updated = await c.env.DB.prepare("SELECT * FROM races WHERE id = ?").bind(raceId).first();
  return c.json(updated);
});
var races_default = races;

// src/routes/drivers.ts
var drivers = new Hono2();
...
// src/routes/rosters.ts
var rosters = new Hono2();
...
var setRosterSchema = external_exports.object({
  driver_order: external_exports.array(external_exports.string().min(2).max(5)).min(1).max(22)
});
rosters.put("/:id/roster", zValidator("json", setRosterSchema), async (c) => {
  const raceId = c.req.param("id");
  const userId = c.get("userId");
  const { driver_order } = c.req.valid("json");
  const race = await c.env.DB.prepare("SELECT status FROM races WHERE id = ?").bind(raceId).first();
  if (!race) {
    return c.json({ error: "Race not found", code: "NOT_FOUND" }, 404);
  }
  ...
  if (existing) {
    ...
    ).bind(orderJson, now, existing.id).run();
    const updated = await c.env.DB.prepare("SELECT * FROM rosters WHERE id = ?").bind(existing.id).first();
    return c.json({
      ...updated,
      driver_order,
    ...
    ).bind(id, userId, raceId, orderJson, now).run();
    return c.json(
      {
        id,
        user_id: userId,
...
var adminRosterSchema = external_exports.object({
  driver_order: external_exports.array(external_exports.string()).min(1).max(22).optional(),
  total_points: external_exports.number().int().optional(),
  breakdown: external_exports.array(external_exports.number()).optional()
});
rosters.patch(
  "/:id/rosters/:rosterId",
  requireAdmin,
  ...
  async (c) => {
    ...
    const body = c.req.valid("json");
    const existing = await c.env.DB.prepare("SELECT * FROM rosters WHERE id = ?").bind(rosterId).first();
    if (!existing) {
      return c.json({ error: "Roster not found", code: "NOT_FOUND" }, 404);
    }
    const updates = {};
    if (body.driver_order !== void 0)
      updates.driver_order = JSON.stringify(body.driver_order);
    if (body.total_points !== void 0)
      updates.total_points = body.total_points;
    if (body.breakdown !== void 0)
      updates.breakdown = JSON.stringify(body.breakdown);
    updates.updated_at = (/* @__PURE__ */ new Date()).toISOString();
    const fields = Object.entries(updates);
    const setClauses = fields.map(([k]) => `${k} = ?`).join(", ");
    const values = fields.map(([, v]) => v);
    await c.env.DB.prepare(`UPDATE rosters SET ${setClauses} WHERE id = ?`).bind(...values, rosterId).run();
    const updated = await c.env.DB.prepare("SELECT * FROM rosters WHERE id = ?").bind(rosterId).first();
    return c.json({
      ...updated,
...
function calculateRosterPoints(driverOrder, results) {
  let total = 0;
  const breakdown = [];
  for (let i = 0; i < driverOrder.length; i++) {
    ...
    let pts = 0;
    if (actualPos !== void 0 && predictedPos === actualPos) {
...
__name(calculateRosterPoints, "calculateRosterPoints");

// src/routes/admin.ts
var admin = new Hono2();
admin.use("*", requireAuth, requireAdmin);
var scoreRaceSchema = external_exports.object({
  results: external_exports.array(
    external_exports.object({
      driver_abbreviation: external_exports.string().min(2).max(5),
      position: external_exports.number().int().min(1).max(30)
    })
  ).min(1)
});
admin.post("/races/:id/score", zValidator("json", scoreRaceSchema), async (c) => {
  ...
  const insertStmts = results.map((r) => {
    ...
    const pts = POSITION_POINTS2[r.position] ?? 0;
  ...
  const rosterUpdates = rostersResult.results.map((roster) => {
    const driverOrder = JSON.parse(roster.driver_order || "[]");
...
admin.delete("/races/:id/results", async (c) => {
  ...
  await c.env.DB.batch(batch);
  return c.json({ ok: true, raceId, rostersReset: rosters2.results.length });
});
...
var patchUserSchema = external_exports.object({
  nickname: external_exports.string().min(1).max(50).trim().optional(),
  total_points: external_exports.number().int().min(0).optional(),
  admin: external_exports.number().int().min(0).max(1).optional()
});
admin.patch("/users/:id", zValidator("json", patchUserSchema), async (c) => {
  ...
  const setClauses = fields.map(([k]) => `${k} = ?`).join(", ");
  const values = fields.map(([, v]) => v);
  await c.env.DB.prepare(`UPDATE users SET ${setClauses} WHERE id = ?`).bind(...values, userId).run();
  const updated = await c.env.DB.prepare(
    "SELECT id, email, given_name, family_name, nickname, total_points, admin, created_at FROM users WHERE id = ?"
  ).bind(userId).first();
  return c.json(updated);
});
var POSITION_POINTS2 = {
  1: 25,
  2: 18,
  3: 15,
  4: 12,
...
app.use(
  ...
  cors({
    origin: (origin) => {
      if (!origin)
        return "*";
      if (origin.includes("localhost") || origin.includes("127.0.0.1") || origin.includes(".pages.dev") || origin.includes("f1fantasy")) {
        return origin;
      }
      return null;
    },
...
app.route("/api/races", races_default);
app.route("/api/races", rosters_default);
app.route("/api/drivers", drivers_default);
app.route("/api/leaderboard", leaderboard_default);
app.route("/api/admin", admin_default);
app.get("/api/health", (c) => c.json({ ok: true, ts: Date.now() }));
app.notFound(
  (c) => c.json({ error: "Not found", code: "NOT_FOUND" }, 404)
);
app.onError((err, c) => {
  console.error(err);
...
// ../../node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260301.1/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// ../../node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260301.1/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// .wrangler/tmp/bundle-ymRyQ0/middleware-insertion-facade.js
...
// ../../node_modules/.pnpm/wrangler@3.114.17_@cloudflare+workers-types@4.20260301.1/node_modules/wrangler/templates/middleware/common.ts
...
// .wrangler/tmp/bundle-ymRyQ0/middleware-loader.entry.ts
